...
├── opmode/
│   ├── Teleop.java            Iterative OpMode driving Follower.manual()
...
These documentation updates are part of the work — include applicable updates in the same commit.
...
4. **Keep `CLAUDE.md` pointing here** so Claude reads the same repository guidance. Maintain shared
   rules in this file rather than duplicating them in `CLAUDE.md`.
